...
## AI 产品落地方法论（我的标志性框架：感知 · 决策 · 执行）
- 感知：视觉/多模态识别。视觉质检沉淀「标注—阈值—置信度分级」范式，可迁移到目标检测、缺陷识别。
- 决策：规则引擎 + 模型预测。「先规则保下限、再模型提上限」的渐进路线，适用预测/调度。
- 执行：人机协同补边界。高置信自动执行、低置信交人工接管 —— 可靠落地的核心。

## 3 个 AI 项目（2025.1 至今，任 AI 产品经理）
1. 碳纤维零部件 AI 视觉质检系统（感知）：商汤视觉 AI + 自建 1200+ 张缺陷样本，AI+人工协同（置信度>80% 判缺陷，50-80% 标疑似），初筛准确率 85%。漏检率 8%→2%（↓75%），质检效率 +40%（日均 500→700 件），人工工作量 -30%。
2. 内部销售 RAG 知识库助手（多模态交互）：200+ 产品文档/报价/历史询盘，Claude API + Pinecone 多路召回与重排，50 个场景评测先行。新销售上手 2 周→3 天，问题响应 30 分→2 分，覆盖率 95%/准确率 92%，销售满意度 3.2→4.5/5。
3. 碳纤维原材料智能采购建议系统（时序预测 · 决策）：3 年订单（12000+ 条）+ 汇率/物流/车展档期 8 个因素，Prophet 预测 90 天需求 + 规则引擎给采购建议。库存周转 60→42 天（↑30%），缺货延期月均 8→2 次（↓75%），库存积压 15→1 天，采购决策效率 +50%；自研成本<5 万（vs 百万级 SAP）。

## 早期产品经理经历（2022.3—2024.12）
- 负责碳纤维汽车/摩托车零部件产品线（车身覆盖件/整流罩/排气管等 200+ SKU）；主导海外品牌从 0→1 落地及销售体系搭建。
- 管理新媒体运营（小红书/抖音/公众号/Facebook/Instagram）+ 阿里国际站；带内容团队（2→4 人）月产 50+ 条内容；阿里国际站询盘转化 +35%，新媒体粉丝累计 10 万+。
